...
#### `AppContextX`: extensión de BuildContext

Se añade una extensión sobre `BuildContext` que expone accesos directos a las propiedades más
comunes del tema activo, eliminando la necesidad de llamar a `Theme.of(context)` en cada widget:

- `theme` — devuelve el `ThemeData` actual.
- `colorScheme` — acceso directo al `ColorScheme`.
- `textTheme` — acceso directo al `TextTheme`.
- `brightness` — devuelve el `Brightness` del tema activo.
- `isDarkMode` — devuelve `true` cuando el brillo es `Brightness.dark`.

#### `AppScaffold`: template base del sistema de diseño

Se incorpora un template que envuelve `Scaffold` aplicando los defaults del sistema de diseño sin
configuración adicional por parte del consumidor:

- Fondo desde `ColorScheme.surface` de forma automática.
- `AppBar` con el tema configurado en `AppAppBarTheme`.
- `SafeArea` automático: aplica `top` solo cuando no hay `AppBar` presente.
- Actualizado en el proyecto example de showcase e incorporado como nueva sección.

#### `AppFeedbackBanner`: banner de retroalimentación contextual

Se añade una molécula que compone `AppIcon` y `AppText` para comunicar estados al usuario. Cuando
`message` es `null`, el banner se oculta con animación mediante `AnimatedSwitcher`. Expone cuatro
constructores de fábrica:

- `AppFeedbackBanner.error` — error de validación o fallo de operación.
- `AppFeedbackBanner.success` — operación completada exitosamente.
- `AppFeedbackBanner.warning` — advertencia no bloqueante.
- `AppFeedbackBanner.info` — información contextual neutral.

#### `AppNavigationBarTheme`: tema para la barra de navegación

Se añade la configuración de tema para la barra de navegación inferior, unificando su apariencia
dentro del sistema de diseño.

#### `AppTextButton`: átomo de botón de texto

Se incorpora un botón de texto que compone `TextButton` con `AppText`, delegando la tipografía al
sistema de variantes existente. Acepta `variant`, `color` y `padding` como parámetros opcionales
para casos de uso específicos.

#### `BookingCard` y `PastBookingCard`: tarjetas de reserva

Se añaden dos moléculas para representar el estado de las reservas en la interfaz:

- `BookingCard` — tarjeta para mostrar información de una reserva próxima.
- `PastBookingCard` — tarjeta en formato horizontal para reservas pasadas o finalizadas, con
  desaturación visual para diferenciarlas del historial activo.

#### `AppTopBar`: barra superior personalizada

Se añade una barra superior que implementa `PreferredSizeWidget` y muestra un título centrado con
`AppText`. Expone los siguientes parámetros:

- `title` — texto centrado renderizado con `AppTextVariant.titleMedium`.
- `actions` — widgets opcionales al final de la barra.
- `showDrawerIcon` — controla la visibilidad del icono de menú; `true` por defecto.
- `onMenuPressed` — callback opcional; si es `null`, abre el drawer automáticamente con
...
? 1 :
widget.maxLines,minLines: widget.minLines,

// Ahora — comportamiento consistente
maxLines: widget.obscureText
? 1 :
widget.maxLines,minLines: widget.obscureText
? 1 :
widget.minLines,
```
...
La carpeta `lib/src/desing_system` ha sido renombrada. Este cambio es un detalle de implementación
interno y **no afecta
...
Lanzamiento inicial del **Eventix App UI Kit**. Un sistema de diseño completo y escalable para
